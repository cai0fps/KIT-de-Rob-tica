    ...
    if(RecOk==1)
    {
      ...
      { 
        ...
        //Serial.println("caso 1");
		  ...
		  uint16_t escalaCinza = (uint16_t)((rx + gx + bx)/3);
		  //Serial.println(escalaCinza);
		  inPotr_Digi[3] = map(escalaCinza, 0, 655, 0, 100);
...
int CorUltra::PegarCorCinza(){
	this->Get_SoftSerialData();
	if(inPotr_Digi[0]!=0x01) {return 9000;}
	else {return inPotr_Digi[3];}
