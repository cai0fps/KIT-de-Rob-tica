...
void TFT_eSPI::fillRect2(int32_t x, int32_t y, int32_t w, int32_t h, uint32_t rect_color)
{
  static uint32_t cor_atual,_x,_y,_w,_h;
  if (_vpOoB) return;
  ...
  if(cor_atual != rect_color || _x != x){
    cor_atual = rect_color;
    _x = x;
    setWindow(x, y, x + w - 1, y + h - 1);
    pushBlock(rect_color, w * h);
  }
